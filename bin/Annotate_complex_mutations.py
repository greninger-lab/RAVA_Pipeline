    ...
    if "," in line:
        ...
        if line.split(",")[4][:-1] == last:
            if not seen_one_complex:
                print(
                    "WARNING: Complex mutation detected (multiple nucleotide changes within the same codon)! Sample="
                    + line.split(",")[0]
                    + " Change1="
                    + line.split(",")[4]
                    + " and Change2="
                    + last_line.split(",")[4]
                    + " this may happen again but future warnings will be suppressed. You can find the complete list in the output folder under complex.log."
                )
...
for line in line_list:
    if line in complex_list:
        g.write(
            ",".join(line.split(",")[:8]) + ",complex," + ",".join(line.split(",")[9:])
        )
    else:
        g.write(line)
g.close()
